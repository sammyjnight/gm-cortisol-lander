...
function CombinedTabbedSection() {
  ...
  return (
    <div className="bg-[var(--color-dark-secondary)] border border-[var(--color-dark-tertiary)] rounded-2xl p-4 md:p-10">
      {/* Tab strip */}
      <div className="grid grid-cols-3 mb-6 md:mb-8 rounded-xl overflow-hidden border border-white/15">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`py-4 md:py-5 text-center text-[15px] md:text-lg font-[800] tracking-wide transition-all border-r border-white/10 last:border-r-0 ${tab === t.id ? "bg-[#3a4255] text-white border-b-[3px] border-b-[var(--color-cyan)]" : "bg-[#252b38] text-white/50 hover:text-white hover:bg-[#2f3645]"}`}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-3" s={0.04}>
            {INGS.map((ing) => (
              <motion.div key={ing.name} variants={cF} className="ing-card" style={{ backgroundImage: `url(/assets/${ing.img})` }}>
                {/* Dose pill top-right */}
                <div className="ing-inner text-center">
                  <h4 className="text-white font-[900] text-[15px] md:text-xl uppercase tracking-wide leading-tight">{ing.name}</h4>
                  <span className="inline-block bg-white/90 text-[var(--color-ink-primary)] font-mono font-semibold text-[10px] md:text-[11px] px-2 py-0.5 rounded mt-1 mb-1 mx-auto">{ing.dose}</span>

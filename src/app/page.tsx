...
function Sec({ mode, children, className = "", id }: { mode: "light" | "dark"; children: React.ReactNode; className?: string; id?: string }) {
  const bg = mode === "light" ? "sec-light" : "sec-dark";
  return <section id={id} className={`${bg} py-16 md:py-24 ${className}`}>{children}</section>;
}

function SN({ n, label, mode = "light" }: { n: string; label: string; mode?: "light" | "dark" }) {
  const c = mode === "light" ? "text-[var(--color-cyan)]" : "text-[var(--color-cyan-bright)]";
  return <p className={`label-mono ${c} mb-4 text-xs`}>{n} &mdash; {label}</p>;
}
...
function Skel({ label, mode = "light", className = "" }: { label: string; mode?: "light" | "dark"; className?: string }) {
  const cls = mode === "light" ? "skel-light" : "skel-dark";
  const txtCls = mode === "light" ? "text-[var(--color-cyan)]" : "text-[var(--color-cyan-bright)]";
  return (
    <div className={`${cls} ${className}`}>
      <p className={`label-mono ${txtCls} opacity-60 text-[10px] z-10 text-center px-4`}>Asset TODO: {label}</p>
    </div>
  );
}
...
function FadeUp({ children, className = "", delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, ease: "easeOut", delay }} className={className}>
      {children}
    </motion.div>
  );
}
...
function Stagger({ children, className = "", s = 0.08 }: { children: React.ReactNode; className?: string; s?: number }) {
  return (
    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={{ visible: { transition: { staggerChildren: s } } }} className={className}>
      {children}
    </motion.div>
  );
}

const cFade = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" as const } } };

function ABar({ value, max, delay = 0, mode = "light" }: { value: number; max: number; delay?: number; mode?: "light" | "dark" }) {
  const ref = useRef<HTMLDivElement>(null);
  const iv = useInView(ref, { once: true, margin: "-60px" });
  return (
    <div ref={ref} className={`w-full rounded-full h-3 overflow-hidden ${mode === "light" ? "bg-[#e5e7eb]" : "bg-[var(--color-dark-secondary)]"}`}>
      <motion.div className="h-3 rounded-full bg-[var(--color-cyan)]" initial={{ width: 0 }} animate={iv ? { width: `${(value / max) * 100}%` } : { width: 0 }} transition={{ duration: 1.2, ease: "easeOut", delay }} />
    </div>
  );
}
...
export default function Page() {
  ...
  useEffect(() => {
    const fn = () => setSticky(window.scrollY > 800);
    ...
    <>
      ...
      <header className="sec-light border-b border-[#e5e7eb] py-4 sticky top-0 z-40 bg-[var(--color-light-primary)]/95 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
          <a href={SHOP} className="text-[var(--color-ink-primary)] font-black text-lg tracking-[0.12em]">GENIUS MIND</a>
          <nav className="hidden md:flex items-center gap-6 text-sm text-[var(--color-ink-secondary)]">
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <FadeUp>
              ...
              <h1 className="text-4xl md:text-5xl lg:text-[3.4rem] font-black leading-[1.06] mb-6 tracking-tight text-[var(--color-ink-primary)]">
                Your Brain Isn&apos;t Tired. It&apos;s Running On What{" "}
                <span className={cyanL}>Cortisol Left Behind.</span>
              </h1>
              <p className="text-[var(--color-ink-primary)] font-semibold text-lg mb-4 leading-snug">
                Every high-pressure year you&apos;ve run this business, cortisol has been wearing down the chemistry your brain needs to think clearly. This is how you restore it.
              </p>
              ...
              <p className="text-[var(--color-ink-primary)] font-semibold mb-2">Replenish what cortisol depleted. Restore the chemistry.</p>
              <p className="text-[var(--color-ink-primary)] font-semibold mb-8">The chaos isn&apos;t going anywhere &mdash; but your brain doesn&apos;t have to keep paying for it.</p>
              <a href="#mechanism" className="btn-secondary inline-flex items-center gap-2">See How It Works <ArrowRight size={16} /></a>
            </FadeUp>
            <FadeUp delay={0.15}>
              <Skel label="High Cortisol Brain vs Low Cortisol Brain — Genius Mind bottle centred" mode="light" className="aspect-[4/3] max-h-[480px]" />
            </FadeUp>
          </div>
        </div>
        <motion.div className="flex justify-center mt-12" animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
          <ChevronDown size={24} className={capL} />
        </motion.div>
      </Sec>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <FadeUp delay={0.1}>
              ...
              <h2 className={`text-3xl md:text-4xl font-black mb-6 leading-tight ${h2D}`}>It&apos;s Not Just Burnout</h2>
              ...
              <Stagger className="space-y-2 mb-6">
                {SYMPTOMS.map((s) => (
                  <motion.div key={s.bold} variants={cFade} className="card-dark card-dark-warn flex items-start gap-3 !py-3 !px-4">
      ...
      {/* ═══ §3+4 WHY EVERYTHING FAILED + WORLD — DARK (merged) ═══ */}
      <Sec mode="dark" className="!pt-0">
        <div className="max-w-4xl mx-auto px-4">
          <FadeUp>
            <SN n="02" label="THE CAUSE" mode="dark" />
            <h2 className={`text-3xl md:text-4xl font-black text-center mb-8 leading-tight ${h2D}`}>Why Everything You&apos;ve Tried Made Logical Sense &mdash; And Still Didn&apos;t Work</h2>
          </FadeUp>
          <div className="grid lg:grid-cols-2 gap-12 items-start mb-12">
            <FadeUp>
              ...
              <p className={`${bodyD} mb-4`}>And the reason is simple:</p>
            ...
            <FadeUp delay={0.1}>
              ...
              <p className={`${bodyD} mb-4`}>The Slack message at 10pm. The funding round. The hire that isn&apos;t working out. The kid who&apos;s sick the day of the board meeting. None of it is life or death. But your nervous system can&apos;t tell the difference.</p>
            </FadeUp>
          </div>
          <FadeUp>
            <p className={`${bodyD} text-center mb-8`}>And the harder you try to think your way out of cortisol-driven cognitive depletion without addressing the chemistry first, the more you&apos;re working against yourself:</p>
          </FadeUp>
          <Stagger className="grid md:grid-cols-3 gap-5">
            {FAILURES.map((f, i) => (
              <motion.div key={f.title} variants={cFade} className="card-dark card-dark-warn">
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <FadeUp delay={0.1}>
              ...
              <p className="text-[var(--color-ink-primary)] font-bold text-xl">That&apos;s exactly what Genius Mind is built to do.</p>
            </FadeUp>
          </div>
        </div>
      </Sec>
      ...
      <Sec mode="dark">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <FadeUp>
              <h2 className={`text-3xl md:text-4xl font-black mb-8 leading-tight ${h2D}`}>Say No to Caffeine Dependency &amp; Nootropic Snake Oil</h2>
              ...
              {ENEMY_CAFF.map((e) => (
                <div key={e.bold} className="card-dark card-dark-warn flex items-start gap-3 !py-3 !px-4 mb-2">
                  <X size={14} className={`${coralD} mt-0.5 shrink-0`} />
                  <p className="text-sm"><strong className="text-white">{e.bold}</strong> <span className={bodyD}>{e.rest}</span></p>
                </div>
              ))}
      ...
      <Sec mode="light" id="mechanism">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <FadeUp>
            <SN n="04" label="THE SOLUTION" />
            <h2 className={`text-3xl md:text-4xl font-black mb-6 leading-tight ${h2L}`}>Cognitive Chemistry Restored. <span className={cyanL}>Sustained Focus, Replenished.</span></h2>
          ...
          <Stagger className="grid md:grid-cols-3 gap-5 mb-12">
            {MECHS.map((m, i) => (
              <motion.div key={m.title} variants={cFade} className="card-light card-light-featured text-left !p-6">
                <p className={`label-mono ${cyanL} text-[10px] mb-3`}>0{i + 1} &mdash; {m.label}</p>
                <motion.div className={`${cyanL} mb-3`} animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>{m.icon}</motion.div>
                <h3 className={`font-bold text-lg mb-2 ${h2L}`}>{m.title}</h3>
          ...
          <FadeUp>
            <div className="max-w-2xl mx-auto">
              <p className={`${bodyL} text-sm mb-4 leading-relaxed`}>Genius Mind is a precision-formulated stack of 16 clinically studied ingredients &mdash; high-ratio botanical extracts, amino acid precursors, and essential cofactors &mdash; designed to support sustained focus throughout the working day. No proprietary blends. No fillers. Every dose transparent and clinically backed.</p>
      ...
      <Sec mode="light" className="sec-light-alt">
        <div className="max-w-6xl mx-auto px-4">
          <FadeUp><SN n="05" label="THE OUTCOMES" /></FadeUp>
          <Stagger className="grid md:grid-cols-3 gap-5">
            {BENEFITS.map((b) => (
              <motion.div key={b.title} variants={cFade} className="card-light text-center">
                <div className={`${cyanL} mb-3 flex justify-center`}>{b.icon}</div>
                <h3 className={`font-bold mb-2 ${h2L}`}>{b.title}</h3>
                <p className={`${bodyL} text-sm`}>{b.desc}</p>
                {b.survey && <p className={`label-mono ${capL} text-[9px] mt-3`}>{b.survey}</p>}
              </motion.div>
            ))}
          </Stagger>
        </div>
      ...
      {/* ═══ §9 16 INGREDIENTS — LIGHT ═══ */}
      <Sec mode="light" id="formula">
        <div className="max-w-6xl mx-auto px-4 text-center">
          <FadeUp>
            <SN n="06" label="THE FORMULA" />
            <h2 className={`text-3xl md:text-4xl font-black mb-3 leading-tight ${h2L}`}>16 Ingredients in 1 Powerful Formula</h2>
            <p className={`label-mono ${cyanL} mb-6`}>Clinically Studied + High-Ratio Extracts</p>
            <div className="flex flex-wrap justify-center gap-4 mb-10">
              {["No proprietary blends", "No fillers", "No synthetic stimulants", "No BS", "No cheap powders"].map((t) => (
                <span key={t} className={`flex items-center gap-1.5 ${bodyL} text-xs`}>
                  <X size={12} className={coralL} />{t}
                </span>
              ))}
            </div>
          </FadeUp>
          <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-4" s={0.04}>
            {INGS.map((ing) => (
              <motion.div key={ing.name} variants={cFade} className="card-light text-left !p-5 flex flex-col justify-end min-h-[180px]">
                <h4 className={`font-bold text-base mb-1 ${h2L}`}>{ing.name}</h4>
                <span className={`label-mono ${cyanL} text-[11px] mb-2 inline-block`}>{ing.dose}</span>
                <p className={`${bodyL} text-xs leading-relaxed`}>{ing.desc}</p>
              </motion.div>
            ))}
          </Stagger>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-wrap justify-center gap-x-10 gap-y-3 text-center">
          {TRUST.map((b) => (
            <div key={b.label} className="flex items-center gap-2">
              <span className={bodyL}>{b.icon}</span>
              <p className={`label-mono ${capL} text-[10px]`}>{b.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <Sec mode="light">
        <div className="max-w-5xl mx-auto px-4">
          <FadeUp>
            ...
            <h2 className={`text-3xl md:text-4xl font-black text-center mb-6 leading-tight ${h2L}`}>How Genius Mind Fits Into A Serious Operator Stack</h2>
            <p className={`${bodyL} text-center max-w-3xl mx-auto mb-10`}>You already take creatine. Probably omega-3. Maybe AG1 or a multi. Magnesium at night. Genius Mind is the chemistry layer &mdash; the missing piece between the lifestyle work you&apos;ve already done and the cognitive output you&apos;re trying to protect.</p>
          </FadeUp>
          <Stagger className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-10">
            {STACK.map((s) => (
              <motion.div key={s.label} variants={cFade} className={`card-light text-center !p-4 ${s.hl ? "card-light-featured !bg-[rgba(8,145,178,0.06)] !border-[var(--color-cyan)]" : ""}`}>
      ...
      <Sec mode="light" className="sec-light-alt">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <FadeUp>
            ...
            <h2 className={`text-3xl md:text-4xl font-black mb-8 leading-tight ${h2L}`}>What Long-Term Customers Actually Report</h2>
          </FadeUp>
          <div className="max-w-2xl mx-auto space-y-5 mb-6">
            {SURVEY.map((s, i) => (
              <FadeUp key={s.label} delay={i * 0.1}>
                ...
                <ABar value={s.count} max={33} delay={i * 0.15} mode="light" />
              </FadeUp>
            ))}
          </div>
      ...
      <Sec mode="dark">
        <div className="max-w-6xl mx-auto px-4 text-center">
          <FadeUp><h2 className={`text-3xl md:text-4xl font-black mb-10 leading-tight ${h2D}`}>What Operators Are Saying</h2></FadeUp>
          <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {VIDS.map((v) => (
              <motion.div key={v.label} variants={cFade} className="skel-dark aspect-[9/16] relative">
                <div className="absolute top-3 left-3 right-3 z-10"><span className="label-mono bg-[var(--color-cyan-bright)] text-[var(--color-dark-primary)] px-3 py-1 rounded text-[10px] font-bold">{v.label}</span></div>
      ...
      <Sec mode="light">
        <div className="max-w-4xl mx-auto px-4">
          <FadeUp>
            ...
            <h2 className={`text-3xl md:text-4xl font-black text-center mb-3 leading-tight ${h2L}`}>How Genius Mind Compares</h2>
            <p className={`${bodyL} text-center mb-10 max-w-xl mx-auto`}>Cognitive infrastructure, not a stimulant hit.</p>
          </FadeUp>
          <FadeUp>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#e5e7eb]">
                    <th className={`py-3 px-4 text-left label-mono text-[10px] ${capL}`}></th>
                    <th className={`py-3 px-4 text-center font-bold ${cyanL} bg-[rgba(8,145,178,0.05)]`}>Genius Mind</th>
                    <th className={`py-3 px-4 text-center ${bodyL}`}>Caffeine Stacks</th>
                    <th className={`py-3 px-4 text-center ${bodyL}`}>Generic Nootropics</th>
                  </tr>
                </thead>
                <tbody>
                  {COMP.map((r, i) => (
                    <tr key={r.label} className={i % 2 === 1 ? "bg-[var(--color-light-secondary)]" : ""}>
                      <td className={`py-3.5 px-4 label-mono text-[10px] ${bodyL}`}>{r.label}</td>
                      <td className="py-3.5 px-4 text-center bg-[rgba(8,145,178,0.05)]"><Check size={18} className={`${cyanL} mx-auto`} /></td>
                      <td className="py-3.5 px-4 text-center">{r.caff === "x" ? <X size={18} className={`${coralL} mx-auto`} /> : <HelpCircle size={16} className={`${capL} mx-auto`} />}</td>
                      <td className="py-3.5 px-4 text-center">{r.generic === "x" ? <X size={18} className={`${coralL} mx-auto`} /> : <HelpCircle size={16} className={`${capL} mx-auto`} />}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Sec mode="light" className="sec-light-alt">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <FadeUp><Skel label="Product shot with benefit callouts (Sharpen Focus / Sustain Energy / Support Memory / Extend Output)" mode="light" className="aspect-square" /></FadeUp>
            <FadeUp delay={0.1}>
              <h2 className={`text-3xl font-black mb-4 leading-tight ${h2L}`}>Cognitive Infrastructure for Operators</h2>
              <p className={`${h2L} font-semibold mb-4`}>Genius Mind is a complete cognitive stack engineered around the Cognisync Tri-Factor &mdash; supporting dopamine pathways, cerebral blood flow, and synaptic signalling with 16 clinically studied ingredients.*</p>
              <ul className="space-y-2 mb-6">
                {BULLETS.map((b) => (
                  <li key={b} className="flex items-start gap-2 text-sm"><Check size={16} className={`${cyanL} mt-0.5 shrink-0`} /><span className={bodyL}>{b}</span></li>
                ))}
              </ul>
              <div className="flex items-center gap-3 mb-4">
                <span className={`text-3xl font-black ${h2L}`}>&pound;24.99</span>
                <span className={`${capL} line-through text-lg`}>&pound;34.99</span>
                <span className={`label-mono ${cyanL} font-bold text-xs border border-[var(--color-cyan)] rounded px-2 py-0.5`}>Save 28%</span>
              </div>
              ...
              <div className="card-light card-light-featured !p-4">
                <p className={`label-mono ${cyanL} text-[10px] mb-2`}>Best Value: 3-Month Supply</p>
      ...
      {/* ═══ §15 DAY 1/30/90 — LIGHT ═══ */}
      <Sec mode="light">
        <div className="max-w-5xl mx-auto px-4">
          <FadeUp>
            <SN n="11" label="THE TIMELINE" />
            <h2 className={`text-3xl md:text-4xl font-black text-center mb-10 leading-tight ${h2L}`}>What Happens After You Start Restoring Cognitive Chemistry</h2>
          </FadeUp>
              ...
              <FadeUp key={t.day} delay={i * 0.2}>
                <div className="card-light">
                  ...
                  <ul className="space-y-2">
                    {t.items.map((item) => (
                      <li key={item} className={`flex items-start gap-2 text-sm ${bodyL}`}>
                        <span className="w-1 h-1 rounded-full bg-[var(--color-cyan)] mt-2 shrink-0" />{item}
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <Sec mode="light" className="sec-light-alt">
        <div className="max-w-5xl mx-auto px-4">
          <FadeUp>
            ...
            <h2 className={`text-3xl md:text-4xl font-black text-center mb-10 leading-tight ${h2L}`}>The Research Behind The Formula</h2>
          </FadeUp>
          <Stagger className="grid md:grid-cols-3 gap-5">
            {RESEARCH.map((c) => (
              <motion.div key={c.ingredient} variants={cFade} className="card-light relative">
                <BookOpen size={14} className={`absolute top-4 right-4 ${capL}`} />
          ...
          <FadeUp>
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="shrink-0">
                <div className="w-28 h-28 rounded-full border-2 border-[var(--color-cyan)] flex items-center justify-center bg-[rgba(8,145,178,0.04)]">
              ...
              <div>
                <h2 className={`text-2xl md:text-3xl font-black mb-3 leading-tight ${h2L}`}>Feel a Massive Difference in 90 Days <span className={cyanL}>Or Your Money Back</span></h2>
      ...
      {/* ═══ §18 FAQ — LIGHT ═══ */}
      <Sec mode="light" className="sec-light-alt" id="faq">
        <div className="max-w-3xl mx-auto px-4">
          <FadeUp>
            <SN n="13" label="QUESTIONS" />
            <h2 className={`text-3xl md:text-4xl font-black text-center mb-10 leading-tight ${h2L}`}>Frequently Asked Questions</h2>
          </FadeUp>
          <div className="space-y-3">
            {FAQS.map((f, i) => (
              <FadeUp key={i} delay={i * 0.04}>
                <div className={`card-light overflow-hidden ${faq === i ? "!border-l-3 !border-l-[var(--color-cyan)]" : ""}`}>
                  <button onClick={() => setFaq(faq === i ? null : i)} className={`w-full text-left p-5 flex items-center justify-between font-semibold text-sm ${h2L} hover:${cyanL} transition-colors`}>
                    <span>{f.q}</span>
      ...
      <Sec mode="light">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <FadeUp>
            ...
            <h2 className={`text-3xl md:text-4xl font-black mb-12 leading-tight ${h2L}`}>Your Starter Kit <span className={cyanL}>Includes:</span></h2>
          </FadeUp>
          <div className="grid lg:grid-cols-2 gap-12 items-center">
      ...
      <div className={`sticky-bar fixed bottom-0 left-0 right-0 bg-[var(--color-dark-primary)]/95 backdrop-blur-md border-t border-[rgba(255,255,255,0.08)] py-3 px-4 z-50 ${sticky ? "visible" : ""}`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <div className="hidden sm:block">
            ...
            <p className={`text-xs ${capD}`}>16 ingredients &bull; 90-day guarantee &bull; Save up to 43%</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden md:block text-right">
              <span className="text-white font-bold">&pound;24.99</span>
              <span className={`${capD} line-through text-xs ml-1`}>&pound;34.99</span>
            </div>
...
const FAILURES = [
  ...
  { title: "Single-ingredient nootropics solve one thing. Focus isn\u2019t one thing.", desc: "Lion\u2019s Mane alone addresses neurogenesis. It doesn\u2019t touch blood flow, dopamine depletion, or synaptic signal. Focus is a multi-mechanism problem." },
...
const MECHS = [
  { label: "BLOOD FLOW", title: "Blood Flow Activation", icon: <Zap size={28} />, desc: "Ginkgo Biloba 50:1, Rosemary 5:1, Panax Ginseng 20:1. Researched for cerebral blood flow, oxygen and nutrient delivery." },
  { label: "NEURON STIMULATION", title: "Neuron Stimulation", icon: <Lightbulb size={28} />, desc: "Lion\u2019s Mane 4:1, L-Tyrosine, Guarana. Studied for nerve growth factor, dopamine precursor support, and clean sustained energy." },
  { label: "NEURON STRENGTHENING", title: "Neuron Strengthening", icon: <Shield size={28} />, desc: "Bacopa Monnieri 11:1, Phosphatidylserine, B-Complex, Zinc. Studied for synaptic communication, memory consolidation, and cellular brain energy." },
];
...
const INGS = [
  ...
  { name: "Rosemary", dose: "20 mg (5:1)", desc: "Researched for neuroprotective properties." },
  { name: "Vitamin B6", dose: "10 mg", desc: "Studied as a cofactor in neurotransmitter synthesis." },
  { name: "Zinc", dose: "10 mg", desc: "Studied for cognitive function and synaptic signalling." },
